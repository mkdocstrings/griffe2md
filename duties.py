...
def check_api(ctx: Context) -> None:
    ...
    ctx.run(
        griffe_check("griffe2md", search_paths=["src"], color=True),
        title="Checking for API breaking changes",
        command="griffe check -ssrc griffe2md",
        nofail=True,
    )
...
def docs_deploy(ctx: Context) -> None:
    ...
    if "pawamoy-insiders/griffe2md" in origin:
        ctx.run("git remote add upstream git@github.com:mkdocstrings/griffe2md", silent=True, nofail=True)
        ctx.run(
